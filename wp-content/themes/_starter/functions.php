...
function _starter_setup() {
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', '_starter' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( '_starter_custom_background_args', array(
 ...
 */
function _starter_content_width() {
	$GLOBALS['content_width'] = apply_filters( '_starter_content_width', 640 );
}
add_action( 'after_setup_theme', '_starter_content_width', 0 );
...
function _starter_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', '_starter' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', '_starter' ),
...
function _starter_scripts() {
	...
	wp_enqueue_script( '_starter-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
